		...
		if(len(arg0)!=0 and len(arg1)!=0 and len(arg2)!=0):
			...
			def bernoullinb(X,Y):
				try:
					...
					buildmodel.partial_fit(X, Y.ravel())
					joblib.dump(buildmodel, '/home/pes1ug19cs285/bernoullimodel.pkl')
				except Exception as e:
					newmodel=BernoulliNB()
					newmodel.partial_fit(X, Y.ravel(),classes=np.unique(Y))
					joblib.dump(newmodel, '/home/pes1ug19cs285/bernoullimodel.pkl')
			def sgd(X,Y):
				...
				try:
					...
					newmodel=linear_model.SGDClassifier()
					newmodel.partial_fit(X, Y.ravel(),classes=np.unique(Y))
					joblib.dump(newmodel, '/home/pes1ug19cs285/linearmodel_SGD.pkl')
			def perceptron(X,Y):
			...
			def clustering(X,Y):
				try:
					...
					kmeans.partial_fit(X,Y.ravel())
					joblib.dump(kmeans, '/home/pes1ug19cs285/clustering.pkl')
			global count		
			bernoullinb(X,Y)
			sgd(X,Y)
			perceptron(X,Y)
